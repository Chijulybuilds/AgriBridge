...
| Concern | How |
| --- | --- |
| **Auth** | Verifies Supabase Auth JWTs (`requireAuth` middleware). Farmers/investors sign in on the frontend; the backend trusts the token. |
...
| POST | `/api/commodities` | ✅ | Farmer submits a commodity |
| GET  | `/api/commodities?wallet=0x..` | ✅ | List a farmer's commodities |
| GET  | `/api/verifier/queue` | ✅ | Pending review queue |
| POST | `/api/verifier/commodities/:id/approve` | ✅ | Approve + mint on-chain |
| POST | `/api/verifier/commodities/:id/reject` | ✅ | Reject on-chain |
